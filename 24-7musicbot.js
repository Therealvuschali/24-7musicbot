...
var Stream = "http://mp3.RadioHamburg.c.nmdn.net/RadioHamburg/Lifestream192.mp3";
        ...
        if (message.member.voiceChannel) {       
            message.member.voiceChannel.join()
              .then(connection => { // Connection is an instance of VoiceConnection
                ...
                connection.playStream('http://mp3.RadioHamburg.c.nmdn.net/RadioHamburg/Lifestream192.mp3');
...
client.on('ready', () => {
    ...
    channel.join().then(connection => { connection.playStream('http://mp3.RadioHamburg.c.nmdn.net/RadioHamburg/Lifestream192.mp3'); })
    console.log('Connected and playing on YGS');
    let channel1 = client.channels.get('344602529533001728');
    channel1.join().then(connection => { connection.playStream('http://mp3.RadioHamburg.c.nmdn.net/RadioHamburg/Lifestream192.mp3'); })
    ...
    const checkNowPlaying = function (err, station) {
        ...
        if (nowplaying != previousplaying) {
            ...
            client.channels.filter(c => c.type === 'voice' && c.members.has(client.user.id)).forEach(async (chan)  => {
                await chan.leave();
                chan.join().then(connection => { connection.playStream('http://mp3.RadioHamburg.c.nmdn.net/RadioHamburg/Lifestream192.mp3'); });
            });
        }
    }
